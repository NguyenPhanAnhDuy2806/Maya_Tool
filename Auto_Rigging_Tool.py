...
def gui():
    ...
    cmds.text('handStyle_Text', label='FK Icon Style:')
    cmds.optionMenu('handIcon_Menu',cc=scriptName + '.iconChanger("hand")')
    cmds.menuItem(l="Circle")
    cmds.menuItem(l="COG")

    cmds.text('pvStyle_Text', label='PoleVector Icon Style:')
    ...
    cmds.text('armColor_Text', l='Step 4: Pick icon color')
    cmds.gridLayout( nr=1, nc=5, cellWidthHeight=(30, 20))
    cmds.iconTextButton('darkBlue_Btn',bgc=(0.05, 0.05, 0.2))
    cmds.iconTextButton('lightBlue_Btn',bgc=(0, 0, 1))
    cmds.iconTextButton('brown_Btn',bgc=(0.6, 0.3, 0.1))
    ...
    cmds.text('PV_Text', l='Step 5: Set IK elbow options')
...
def armIconScale():
    ...
    # Circle
    handCircle= cmds.circle( n='hand_circle_curve', nr=(1, 0, 0), sw=360, c=(0, 0, 0),r=1,d=3,ut=0,tol=0.01,s=16)
    cmds.rotate( 0, 0, '-90deg', handCircle, pivot=(0, 0, 0) )
...
def ikSetup(armType, oriName, labelName, iconColor, selected):
    ...
    #Variables
    cmds.radioButtonGrp("addPVElbow_Btn", q=True, sl=True)
    if armType == 3:
        ik_joints = []
